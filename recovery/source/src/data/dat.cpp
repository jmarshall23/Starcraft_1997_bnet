...
constexpr std::array<DatFieldShape, 48> kUnitShapes{{
    {1, 228}, {2, 228}, {2, 228}, {2, 98},  {4, 228}, {1, 228}, {1, 228}, {2, 228},
    {4, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228},
    {1, 228}, {1, 228}, {1, 228}, {4, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228},
    {1, 228}, {1, 228}, {2, 106}, {2, 228}, {2, 228}, {2, 106}, {2, 106}, {2, 106},
    {2, 106}, {2, 456}, {2, 196}, {2, 912}, {2, 228}, {2, 228}, {2, 228}, {2, 228},
    {2, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228}, {1, 228},
}};
...
constexpr std::array<DatFieldShape, 6> kFlingyShapes{{
    {2, 175}, {4, 175}, {2, 175}, {1, 175}, {1, 175}, {1, 175},
}};

constexpr std::array<DatFieldShape, 5> kSpriteShapes{{
    {2, 267}, {1, 164}, {1, 267}, {1, 164}, {1, 164},
}};

constexpr std::array<DatFieldShape, 16> kImageShapes{{
    {4, 590}, {1, 590}, {1, 590}, {1, 590}, {1, 590}, {1, 590}, {1, 590}, {1, 590},
    {4, 590}, {4, 590}, {4, 590}, {4, 590}, {4, 590}, {4, 590}, {4, 590}, {1, 590},
}};
...
bool load_dat(
    ...
    const std::array<DatFieldShape, Size>& shapes,
    DatTable& output,
    std::size_t& total,
    std::string& failed_asset) {
  std::vector<std::uint8_t> payload;
  if (!storm.load_file(path, payload) || !output.unpack(payload, shapes.data(), shapes.size())) {
    failed_asset = path;
    return false;
  }
  total += payload.size();
  ...
  try {
    ...
    for (std::size_t index = 0; index < shape_count; ++index) {
      const std::size_t field_size =
          static_cast<std::size_t>(shapes[index].element_width) * shapes[index].element_count;
    ...
    for (std::size_t index = 0; index < shape_count; ++index) {
      ...
      field.bytes_.assign(payload.begin() + cursor, payload.begin() + cursor + field_size);
      cursor += field_size;
...
std::size_t DatTable::field_count() const noexcept {
  return fields_.size();
}

std::size_t DatTable::payload_size() const noexcept {
  return payload_size_;
}

StringTableView::StringTableView(
  ...
  const std::size_t header_size = sizeof(std::uint16_t) +
...
bool StringTableView::valid() const noexcept {
  return valid_;
}
...
std::string_view StringTableView::one_based(const std::uint16_t string_id) const noexcept {
...
bool CoreDataSet::load(runtime::StormModule& storm) noexcept {
  ...
  if (!load_dat(storm, R"(arr\units.dat)", kUnitShapes, units_, total_payload_bytes_, failed_asset_) ||
      !load_dat(storm, R"(arr\weapons.dat)", kWeaponShapes, weapons_, total_payload_bytes_, failed_asset_) ||
      !load_dat(storm, R"(arr\flingy.dat)", kFlingyShapes, flingy_, total_payload_bytes_, failed_asset_) ||
      !load_dat(storm, R"(arr\sprites.dat)", kSpriteShapes, sprites_, total_payload_bytes_, failed_asset_) ||
      !load_dat(storm, R"(arr\images.dat)", kImageShapes, images_, total_payload_bytes_, failed_asset_) ||
      !load_dat(storm, R"(arr\mapdata.dat)", kMapDataShapes, mapdata_, total_payload_bytes_, failed_asset_) ||
      !load_dat(storm, R"(arr\portdata.dat)", kPortraitShapes, portraits_, total_payload_bytes_, failed_asset_)) {
    return false;
  }
  ...
  const StringTableView portrait_strings{
      portrait_strings_.data(), portrait_strings_.size()};
...
const std::vector<std::uint8_t>& CoreDataSet::image_strings() const noexcept { return image_strings_; }
const std::vector<std::uint8_t>& CoreDataSet::iscript() const noexcept { return iscript_; }
const std::vector<std::uint8_t>& CoreDataSet::map_strings() const noexcept { return map_strings_; }
std::size_t CoreDataSet::total_payload_bytes() const noexcept { return total_payload_bytes_; }
const std::string& CoreDataSet::failed_asset() const noexcept { return failed_asset_; }

bool CoreDataSet::extract_unit_traits(starcraft::lang::UnitTraitsTable& traits) const noexcept {
  ...
  if (race == nullptr || provided == nullptr || required == nullptr || flags == nullptr ||
      score == nullptr) {
    return false;
  }
...
std::string CoreDataSet::image_grp_path(const std::uint16_t image_id) const {
  ...
  const StringTableView strings{image_strings_.data(), image_strings_.size()};
  const std::string_view relative = strings.one_based(static_cast<std::uint16_t>(string_id));
  if (relative.empty()) {
    return {};
  }
...
bool CoreDataSet::image_render_traits(
  ...
  std::uint8_t turns{};
  if (graphics_turns == nullptr || draw_function == nullptr || remapping == nullptr ||
      !graphics_turns->value(image_id, turns) ||
...
bool CoreDataSet::unit_image_id(
  ...
  return unit_graphics != nullptr && flingy_sprite != nullptr && sprite_image != nullptr &&
...
bool CoreDataSet::weapon_image_id(
    const std::uint16_t weapon_type,
    std::uint16_t& image_id) const noexcept {
  ...
  return weapon_graphics != nullptr && flingy_sprite != nullptr &&
         sprite_image != nullptr && weapon_graphics->value(weapon_type, flingy_id) &&
         flingy_id < 175U && flingy_sprite->value(flingy_id, sprite_id) &&
         sprite_image->value(sprite_id, image_id) && image_id < 590U;
}

bool CoreDataSet::unit_placement_size(
    const std::uint16_t unit_type,
    std::uint16_t& width,
  ...
  return placement != nullptr &&
         ...
         placement->value(static_cast<std::size_t>(unit_type) * 2U + 1U, height) &&
         width != 0 && height != 0;
}

bool CoreDataSet::unit_collision_extents(
    const std::uint16_t unit_type,
    std::uint16_t& left,
    std::uint16_t& top,
    std::uint16_t& right,
  ...
  // CUnit placement search. StarCraft.exe place_unit.cpp::sub_47FBF0 reads the matching
  // left/top and right/bottom arrays while settling freshly created units.
  ...
  return extents != nullptr && extents->value(base, left) &&
         extents->value(base + 1U, top) &&
         extents->value(base + 2U, right) &&
         extents->value(base + 3U, bottom);
}

bool CoreDataSet::unit_addon_position(
    const std::uint16_t unit_type,
    std::int16_t& x,
...
bool CoreDataSet::unit_build_time(
    const std::uint16_t unit_type,
...
bool CoreDataSet::unit_movement_traits(
  ...
  if (unit_graphics == nullptr || top_speeds == nullptr || accelerations == nullptr ||
      turn_speeds == nullptr || direction_thresholds == nullptr ||
      movement_controls == nullptr || !unit_graphics->value(unit_type, flingy_id) ||
  ...
  return traits.top_speed != 0 && traits.acceleration != 0 && traits.turn_speed != 0;
}

bool CoreDataSet::unit_is_building(
    const std::uint16_t unit_type,
...
bool CoreDataSet::unit_simulation_traits(
  ...
  const DatField* const build_times = units_.field(39);
  if (maximum_life == nullptr || ground_weapons == nullptr || flags == nullptr ||
      armor_classes == nullptr || armor == nullptr || mineral_costs == nullptr ||
      gas_costs == nullptr || build_times == nullptr ||
      ...
      !build_times->value(unit_type, traits.build_time) || traits.max_hit_points == 0) {
    return false;
  }
  ...
  return maximum_ranges != nullptr && damage_classes != nullptr && damages != nullptr &&
         cooldowns != nullptr &&
         maximum_ranges->value(traits.ground_weapon, traits.ground_weapon_range) &&
         damage_classes->value(
             traits.ground_weapon, traits.ground_weapon_damage_class) &&
         damages->value(traits.ground_weapon, traits.ground_weapon_damage) &&
         cooldowns->value(traits.ground_weapon, traits.ground_weapon_cooldown) &&
         traits.ground_weapon_damage != 0 && traits.ground_weapon_cooldown != 0;
}

std::string CoreDataSet::unit_portrait_path(
    const std::uint16_t unit_type,
  ...
  const StringTableView strings{portrait_strings_.data(), portrait_strings_.size()};
...
std::string CoreDataSet::map_archive_path(const std::uint16_t map_id) const {
  ...
  std::uint32_t string_id{};
  if (names == nullptr || !names->value(map_id, string_id) || string_id > 0xFFFFU) {
    return {};
  }
